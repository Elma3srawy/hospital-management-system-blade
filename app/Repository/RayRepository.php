...
class RayRepository implements RayInterface
{
    ...
    public function store(Mixed $request){
        ...
        try
        {
            DB::beginTransaction();
                ...
                DB::table('rays')->insert($validated->all());
    ...
    */
    public function update(Mixed $request){
        try
        {
            ...
            DB::table('rays')->where('id',$request->ray_id)->update(['description'=> $request->description]);
    ...
    */
    public function destroy(){
        ...
        try
        {
            DB::table('rays')->where('id', request('ray_id'))->delete();
            return back()->with('success' , __('flasher::messages.success'));
